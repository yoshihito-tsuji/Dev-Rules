...
あなたが新しいプロジェクトに参加する際、または既存プロジェクトで作業を再開する際は、以下の手順を厳守してください：
...
1. **設計なしに実装しない**: 新機能や大きな変更は必ずCodexの設計を経由
2. **実装は設計に基づく**: Claude Codeは設計書に基づいて実装
3. **最終決定はYoshihitoさん**: 重要な方針変更は必ずYoshihitoさんの承認を得る
4. **すべてを記録する**: 決定事項、提案、実装内容をドキュメント化
