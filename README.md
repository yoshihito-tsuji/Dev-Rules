...
このリポジトリは、Yoshihitoさんのすべてのプロジェクトで使用する四者協働開発方法論の永続的なアーカイブです。

> **2026-05-19 更新**: 設計担当として **Codex を復活**（並行運用）し、**xangi システム**（Discord 上の相談役、キャラクター名「ロコ」）を 4 者目として追加しました。詳細は本ドキュメント「協働体制」セクション、および各役割定義（[docs/team_ops/](docs/team_ops/)）を参照してください。
...
#### Claude Code（設計・プランモード）の場合（参考・旧体制）

1. 次に `docs/team_ops/claude_code_design_role.md` を読んでください
2. 現在は Codex を設計担当に戻したため、プランモードは補助的な位置づけです

#### xangi システム（相談役、Discord 上のキャラクター名「ロコ」）の場合

1. 次に `docs/team_ops/xangi_system_role.md` を読んでください
2. xangi リポの [docs/dev-methodology.md](https://github.com/yoshihito-tsuji/xangi/blob/main/docs/dev-methodology.md) も合わせて確認してください
...
- **役割の明確化**: Codex（設計・実装案）、Claude Code（実装）、xangi システム（相談役）、Yoshihito さん（最終決定）
...
### 役割分担（2026-05-19 改訂・四者協働）

| 担当者 | 主な役割 | 責任範囲 |
|--------|---------|---------|
| **Yoshihitoさん** | プロダクトオーナー（顧客） | 最終意思決定、要件定義、方針決定、管理権限 |
| **xangi システム** | 詳しい相談役（Discord 上のキャラクター名「ロコ」） | 技術アドバイス、状況確認、Yoshihito さんと Codex / Claude Code の中継 |
| **Codex** | 設計・実装案担当 | 要件分析、システム設計、技術選定、実装計画策定 |
| **Claude Code（実装・通常モード）** | 実装担当 | Codex の設計に基づくコーディング、テスト、デバッグ、Git 操作 |

> **補足**: Claude Code（設計・プランモード）の体制は 2026-03-31 〜 2026-05-19 の間に運用されていましたが、現在は Codex を設計担当に戻しています。プランモードは引き続き補助的に利用可能ですが、主担当は Codex です。

### 協働フロー（A 方式・非同期協働）

```
Yoshihitoさん（顧客・最終意思決定）
    │
    ├─→ xangi システム（相談役）         … 状況確認・追加要望の中継
    │
    ├─→ Codex（設計・実装案）
    │       │
    │       │ 設計書・実装案（ノート / PR / コメント）
...
**基本方針**: Codex と Claude Code は**非同期で協働**します。Codex がノート / PR / コメントで設計案を残し、Claude Code がそれを読んで実装します。Claude Code から Codex への質問は Yoshihito さん経由で伝達します（直接やり取りはしません）。

### 重要ルール

1. **顧客は Yoshihitoさん**: 管理権限は常にここ。AI 側で勝手に方針を決めない、スコープを広げない
2. **すべて日本語で記述**: AI応答、コメント、ドキュメントは必ず日本語で記述すること
3. **設計なしに実装しない**: 新機能や大きな変更は必ず Codex の設計を経由
4. **実装は設計に基づく**: Claude Code は Codex の設計書に基づいて実装
5. **xangi システム経由の確認・要望にも応じる**: ただし最終的な指示権限は Yoshihito さんにある
...
### xangi システム（相談役 AI、Discord 上のキャラクター名「ロコ」）

#### 責務

- Yoshihito さんの隣にいる**詳しい相談役**として、技術アドバイス・状況確認を行う
- Codex / Claude Code の作業状況を Yoshihito さんへ要約・報告
- Yoshihito さんからの追加要望を Codex / Claude Code に中継
- 自分自身で実装・設計を巻き取らない（軽量な調査・コマンド実行は可）

#### 成果物

- Discord 上の対話ログ（xangi リポの `memory/` ディレクトリで管理）
...
1. **Yoshihito さん**: 要件を Codex に伝える（必要に応じて xangi システムに相談）
2. **Codex（設計・実装案）**:
   - 要件を分析
   - 設計書を作成（`docs/design/`）
   - 実装計画を策定
   - Yoshihito さんに承認依頼（ノート / PR / コメント形式で残す）
3. **Yoshihito さん**: 設計を承認
4. **Claude Code（実装）**:
   - Codex の設計書を読んで実装
   - テスト実行
   - ログ記録（`LOG/YYYY-MM-DD.md`）
   - Git commit & push
   - Codex への質問が必要なら Yoshihito さん経由で伝達
5. **Yoshihito さん**: 実装結果を確認・承認

### バグ修正フロー

1. **Yoshihito さん**: バグ内容を Claude Code に伝える（xangi システム経由でも可）
2. **Claude Code（実装）**:
   - 軽微なバグ: 直接修正
   - 設計変更が必要: Codex に相談（Yoshihito さん経由）
3. **Codex（設計）**（設計変更が必要な場合）:
   - 修正方針を設計
   - Yoshihito さんに承認依頼
4. **Claude Code（実装）**: 修正実装・テスト・ログ記録
5. **Yoshihito さん**: 修正結果を確認
...
- [docs/team_ops/xangi_system_role.md](docs/team_ops/xangi_system_role.md) - xangi システム（相談役、Discord 上「ロコ」）役割定義
...
**Q: xangi システムとは何ですか？**
A: Discord 上で常駐する AI アシスタントで、Yoshihito さんの**詳しい相談役**として機能します。Discord 上のキャラクター名は「ロコ」ですが、Dev-Rules 上の正式表記は xangi システムです。Codex / Claude Code の作業状況確認や追加要望の中継を担います。
...
A: 誰が誰に向けて話しているかを明確にし、コミュニケーションの混乱を防ぐためです。4 者協働になり From/To の重要性はさらに増しています。
...
| Claude Code（設計/実装）・Yoshihitoさんの協働方法 | Claude Codeの高度な活用方法 |
