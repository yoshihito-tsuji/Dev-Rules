...
> **2026-05-29 更新**: 四者協働を基盤にしつつ、**xangi システム**（Xangi 上で動作する **LOCO（ロコ）**）が、要件のかみ砕き・対話の整理・追加確認を担う運用へ整理しました。詳細は本ドキュメント「協働体制」セクション、および各役割定義（[docs/team_ops/](docs/team_ops/)）を参照してください。
...
#### xangi システム（Xangi 上で動作する LOCO / ロコ）の場合
...
3. 必要に応じて、Codex / Claude Code の説明をかみ砕き、Yoshihitoさんが判断しやすい形に整理してください
...
### 役割分担（2026-05-29 改訂・四者協働）

| 担当者 | 主な役割 | 責任範囲 |
|--------|---------|---------|
| **Yoshihitoさん** | プロダクトオーナー（顧客） | 最終意思決定、要件定義、方針決定、管理権限 |
| **xangi システム / LOCO** | 詳しい相談役・整理役（Xangi 上で動作するエージェント、通称「ロコ」） | 技術アドバイス、状況確認、要件のかみ砕き、Yoshihito さんと Codex / Claude Code の中継、追加確認 |
| **Codex** | 設計・実装案担当 | 要件分析、システム設計、技術選定、実装計画策定 |
| **Claude Code（実装・通常モード）** | 実装担当 | Codex の設計に基づくコーディング、テスト、デバッグ、Git 操作 |

> **補足**: Claude Code（設計・プランモード）の体制は 2026-03-31 〜 2026-05-19 の間に運用されていましたが、現在は Codex を設計担当に戻しています。プランモードは引き続き補助的に利用可能ですが、主担当は Codex です。xangi システム / LOCO は、対話の整理や追加確認を担う同一エージェントです。
...
Yoshihitoさん（顧客・最終意思決定）
    │
    ├─→ xangi システム / LOCO（相談役） … 状況確認・要件整理・追加要望の中継
    │
    ├─→ Codex（設計・実装案）
    │       │
    │       │ 設計書・実装案（ノート / PR / コメント）
...
**基本方針**: Codex と Claude Code は**非同期で協働**します。Codex がノート / PR / コメントで設計案を残し、Claude Code がそれを読んで実装します。xangi システム / LOCO は、対話の要約、論点整理、追加確認を行い、Yoshihito さんが判断しやすい形に整えます。Claude Code から Codex への質問は Yoshihito さん経由で伝達します（直接やり取りはしません）。
...
5. **xangi システム / LOCO による整理・追加確認に応じる**: ただし最終的な指示権限は Yoshihito さんにある
6. **すべてを記録する**: 決定事項、提案、実装内容をドキュメント化
7. **大きな変更は、xangi システム / LOCO でかみ砕きながら確認する**: 見落としと認識ズレを減らす
...
1. **Yoshihito さん**: 要件を Codex に伝える（必要に応じて xangi システム / LOCO に相談）
2. **Codex（設計・実装案）**:
   - 要件を分析
   - 設計書を作成（`docs/design/`）
   - 実装計画を策定
...
5. **xangi システム / LOCO（必要に応じて）**:
   - 大きな変更やリスクの高い変更をかみ砕いて追加確認
   - 仕様漏れ、回帰、セキュリティの観点を整理して共有
6. **Yoshihito さん**: 実装結果を確認・承認

### バグ修正フロー

1. **Yoshihito さん**: バグ内容を Claude Code に伝える（xangi システム / LOCO 経由でも可）
2. **Claude Code（実装）**:
   - 軽微なバグ: 直接修正
...
5. **xangi システム / LOCO（必要に応じて）**: 変更後の確認、影響範囲の再点検
6. **Yoshihito さん**: 修正結果を確認
