...
| 担当者 | 主な役割 | 責任範囲 |
|--------|---------|---------|
| **Yoshihitoさん** | プロダクトオーナー | 最終意思決定、要件定義、方針決定、管理権限 |
| **Claude Code Opus（設計担当）** | 設計担当 | アーキテクチャ設計、技術選定、実装方針策定、agmsg経由で実装担当に仕様を渡す |
| **Claude Code Sonnet（実装担当）** | 実装担当 | Opusの設計に基づくコーディング、テスト、デバッグ、Git操作 |
| **agmsg+agmsg-guard** | エージェント間通信基盤 | OpusとSonnet間のメッセージ配信、無限ループ防止（最大20往復） |
...
Yoshihitoさん（最終意思決定）
    │
    ▼
Claude Code Opus（設計担当）
    │  agmsg でメッセージ送信
    ▼
Claude Code Sonnet（実装担当）
    │  実装完了後 DONE 送信
    ▼
agmsg-guard（監視）… 上限超過・タイムアウトで STOP
```

**基本方針**: Opus と Sonnet は **agmsg 経由でメッセージを交換**します。5往復以内での収束を目標とし、完了時は Sonnet が `DONE` を送信します。agmsg-guard が対話を監視し、上限（デフォルト20往復）またはタイムアウトで強制停止します。
...
5. **5往復以内での収束**: agmsg でのやり取りは5往復以内を基準とする
